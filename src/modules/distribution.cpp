...
Tensor Normal::get_log_prob(const Tensor& actions) const {
  ...
  return -0.5 *
         (torch::log(2. * M_PI * var) + (actions - this->mean_).square() / var);
}
...
Tensor Normal::get_kl(const DictTensor& old_kl_params) const {
  ...
  return torch::sum(torch::log(new_std / old_std) +
...
DictTensor Normal::get_kl_params() const {
  return {{"mean", this->mean_.clone()}, {"std", this->std_.clone()}};
}
...
void Beta::update(const Tensor& hidden_output) {
  ...
  const Tensor& total =
      torch::relu(this->mean_ * (1. - this->mean_) /
                      this->std_.expand_as(this->mean_).square() -
                  1.) +
      1e-07;
...
Tensor Beta::get_entropy() const {
  ...
  return torch::lgamma(alpha_beta) - torch::lgamma(this->alpha_) -
         torch::lgamma(this->beta_) +
         (this->alpha_ - 1.) *
             (torch::digamma(this->alpha_) - torch::digamma(alpha_beta)) +
         (this->beta_ - 1.) *
             (torch::digamma(this->beta_) - torch::digamma(alpha_beta));
}
...
Tensor Beta::get_kl(const DictTensor& old_kl_params) const {
  ...
  return torch::sum(
      torch::lgamma(old_alpha + old_beta) - torch::lgamma(old_alpha) -
          torch::lgamma(old_beta) +
          (old_alpha - alpha) * (torch::digamma(old_alpha) -
                                 torch::digamma(old_alpha + old_beta)) +
          (old_beta - beta) *
              (torch::digamma(old_beta) - torch::digamma(old_alpha + old_beta)),
      /*dim=*/-1);
}

DictTensor Beta::get_kl_params() const {
  return {{"alpha", this->alpha_.clone()}, {"beta", this->beta_.clone()}};
}
