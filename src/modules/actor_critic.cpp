...
namespace modules{
    ...
    Actor::Actor(const int& num_observations, const int& num_actions, const int& depth,
        ...
        this->network_ = utils::create_MLP(num_observations, num_actions, width, depth, activation);
        if (distribution_type == "normal") this->distribution_ = std::make_shared<Normal>(num_actions, init_noise_std, device);
        else if (distribution_type == "beta") {
            this->distribution_ = std::make_shared<Beta>(num_actions, init_noise_std, device);
            this->network_->push_back(torch::nn::Sigmoid());
        }
        else throw std::invalid_argument("Invalid distribution type");
    ...
    Critic::Critic(const int& num_observations, const int& depth,
        ...
        const int& width = utils::next_power_of_2(num_observations);
        this->network_ = utils::create_MLP(num_observations, 1, width, depth, activation);
