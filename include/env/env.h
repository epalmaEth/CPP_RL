...
class Env {
 public:
  explicit Env(const unsigned int& num_envs, int seed, const Device& device)
      : num_envs_(num_envs), device_(device) {
    if (seed < 0) seed = this->sample_random_seed_();
    torch::manual_seed(seed);
    this->all_indices_ = torch::ones({num_envs}).to(torch::kBool).to(device);
  }
  ...
  std::pair<Tensor, DictTensor> reset(unsigned int seed,
  ...
  unsigned int get_num_envs() const { return this->num_envs_; }
  Device get_device() const { return this->device_; }
  Tensor get_state() const { return this->state_; }
  Tensor get_observation() const { return this->step_result_.obs; }
