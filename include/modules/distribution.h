...
namespace modules {
    ...
    public:
        Distribution(const int& input_size, const double& init_noise_std, const Device& device) {
    ...
    public:
        ...
        Beta(const int& input_size, const double& init_noise_std, const Device& device): Distribution(input_size, init_noise_std, device) {
        ...
        void update(const Tensor& hidden_output) override;
        Tensor get_sample() const override;
        Tensor get_mode() const override { return (this->alpha_ - 1.)/(this->alpha_ + this->beta_ - 2.); }
        Tensor get_log_prob(const Tensor& actions) const override;
        Tensor get_entropy() const override;
        Tensor get_kl(const DictTensor& old_kl_params) const override;
        DictTensor get_kl_params() const override;
