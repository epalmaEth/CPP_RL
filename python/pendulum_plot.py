...
def setup_pendulum_elements(ax_main, theta, rod_length, rod_width):
    ...
    rod = Polygon(
        transformed_coords, closed=True, fill=True, edgecolor=red, facecolor=red
    )
    rod_pivot = Circle((0, 0), rod_width / 2, edgecolor=red, facecolor=red)
    rod_end = Circle(
        rotate(rod_length, 0, theta[0] + np.pi / 2),
        rod_width / 2,
        edgecolor=red,
        facecolor=red,
    )
...
def update_plot(
    frame, rod, rod_end, reward_line, action_line, theta, coords, actions, rewards
):
